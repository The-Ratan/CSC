  ...
  const senddata = (evt) => {
    ...
    if (isConfirmed) {
      ...
      if (fir_content_img && fir_content_img.size > 200 * 1024) {
        toast.error("Fir Contnet Image size should be less than 200KB");
        return;
      }
      ...
      axios
        .post(`${apiurl[0].apiUrl}website/add_fir_details`, formdata, options)
        .then((response) => {
          setsimbolstatusstatus(false);
          setloaderstatus(true);
          toast.success(response.data.msg);
          setTimeout(() => {
            window.location.reload("/fir1");
          }, 3000);
        })
        .catch((error) => {});
        ...
        <div id="content">
          <div className="content-admin-main">
            ...
            <div className="aon-admin-heading">
              <h4>
                <strong>Please Fill this detail for FIR</strong>
              </h4>
            </div>
                          ...
                          <div className="radio-inline-box service-perform-list">
                            <div className="checkbox sf-radio-checkbox sf-radio-check-2">
                              <input
                                required
                                onChange={(e) =>
                                  setfir_content_img(e.target.files[0])
                                }
                                type="file"
                                accept="image/*"
                                name="abc"
