...
import secureLocalStorage from "react-secure-storage";
  ...
  const Renewal1 = () => {
    ...
    const [pancard_img, setpancardimage] = useState("");
    const [tenth_marksheet_img, settenth_marksheet] = useState("");
    const [old_passport_or_fir_img, setold_passport_or_fir_img] = useState("");
    ...
    const senddata = (evt) => {
        ...
        if (isConfirmed) {
          if (pancard_img && pancard_img.size > 200 * 1024) {
            toast.error("Pancard Image size should be less than 200KB");
            return; 
          }
          ...
          if (tenth_marksheet_img && tenth_marksheet_img.size > 200 * 1024) {
            toast.error("10th Marksheet Image size should be less than 200KB");
            return; 
          }
          
          
          if (old_passport_or_fir_img && old_passport_or_fir_img.size > 200 * 1024) {
            toast.error("Old Passport Image size should be less than 200KB");
            return; 
          }
          ...
          formdata.append("pancard_img", pancard_img);
    
          formdata.append("tenth_marksheet_img", tenth_marksheet_img);
          formdata.append("old_passport_or_fir_img", old_passport_or_fir_img);
          ...
          axios
            .post(
              `${apiurl[0].apiUrl}website/add_renewel_passport_details`,
              formdata,
              options
            )
            .then((response) => {
              ...
              setTimeout(() => {
                window.location.reload("/renewal1");
              }, 3000);
            })
            .catch((error) => {
          ...
          <div id="content">
            <div className="content-admin-main">
              ...
              <div className="aon-admin-heading">
                <h4>
                  <strong>Please Fill this detail for Renewal Passport</strong>
                </h4>
              </div>
                            ...
                            <div className="radio-inline-box service-perform-list">
                              <div className="checkbox sf-radio-checkbox sf-radio-check-2">
                                <input
                                  required
                                  onChange={(e) =>
                                    settenth_marksheet(e.target.files[0])
                                  }
                                  type="file"
                                  accept="image/*"
                                />
                            ...
                            <div className="radio-inline-box service-perform-list">
                              <div className="checkbox sf-radio-checkbox sf-radio-check-2">
                                <input
                                  required
                                  onChange={(e) =>
                                    setold_passport_or_fir_img(e.target.files[0])
                                  }
                                  type="file"
                                  accept="image/*"
                                />
