...
const Adminsinglefir = () => {
  const [pancard, setpancard] = useState();

  let id = secureLocalStorage.getItem("fir");
  ...
  const deletepancard = () => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}admin/single_fir_data`, Userdata ,options)
      .then((res) => {
...
const formattedDateTime = new Date(createdAt).toLocaleString(undefined, {
  ...
  const senddata = () => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}admin/fir_document`, formdata, options)
      .then((response) => {
    ...
    <>
      ...
      <section className="content">
        <div className="page-heading">
          <h1>Fir Upload Date - {formattedDateTime}</h1>
          <ol className="breadcrumb">
            <li>
              <Link to="/deashboard">Dashboard</Link>
            </li>

            <li className="active">Fir</li>
          </ol>
        </div>
