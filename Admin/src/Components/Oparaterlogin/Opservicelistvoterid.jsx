...
const Opservicelistvoterid = () => {
  ...
  const voterapprove = (item) => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}subadmin/aprove_votercard`, userdata ,options)
      .then((res) => {
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <tbody>
                    {accessdata?.votercard?.map((list,index)=>{
                        return(
<tr>
                        <td>{index + 1}</td>
                        <td>
    ...
    <button
      onClick={() => {
       secureLocalStorage.setItem("opvoter", list?._id);
        Navigate('/opvoterdetails');
      }}
      type="button"
...
export default Opservicelistvoterid;
