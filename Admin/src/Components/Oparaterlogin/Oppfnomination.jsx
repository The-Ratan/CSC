...
const Oppfnomination = () => {
  const [pancard, setaccessdata] = useState();

  let accessdatadid = secureLocalStorage.getItem("oppfnomination");
  ...
  const useraccessdata = () => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}subadmin/single_pf_nomination_data`, userdata ,options)
      .then((res) => {
  ...
  const senddata = () => {
    ...
    axios
      .post(
        `${apiurl[0].apiUrl}subadmin/pfnomination_document`,
        formdata,
        options
      )
      .then((response) => {
