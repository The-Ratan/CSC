...
const Opincomedetails = () => {
  const [pancard, setaccessdata] = useState();

  let accessdatadid = secureLocalStorage.getItem("opincome");
  ...
  const useraccessdata = () => {
    ...
    axios
      .post(
        `${apiurl[0].apiUrl}subadmin/single_income_certificate_data`,
        userdata , options
      )
      .then((res) => {
  ...
  const senddata = () => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}subadmin/income_document`, formdata, options)
      .then((response) => {
...
export default Opincomedetails;
