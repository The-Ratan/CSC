...
const Opfreshpassport = () => {
  const [pancard, setaccessdata] = useState();

  let accessdatadid = secureLocalStorage.getItem("opfreshpassport");
  ...
  const useraccessdata = () => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}subadmin/single_fresh_passport_data`, userdata ,options)
      .then((res) => {
  ...
  const senddata = () => {
    ...
    axios
      .post(
        `${apiurl[0].apiUrl}subadmin/fresh_passport_document`,
        formdata,
        options
      )
      .then((response) => {
...
export default Opfreshpassport;
