...
const Opservicelistudid = () => {
  ...
  const udidapprove = (item) => {
    ...
    axios
      .post(`${apiurl[0].apiUrl}subadmin/aprove_udidcard`, userdata ,options)
      .then((res) => {
    ...
    <div>
      ...
      <section className="content">
        ...
        <div className="panel panel-default">
          <div className="panel-heading">SERVICES LIST UDID CARD</div>
          <div className="panel-body">
            <div className="table-responsive">
            {accessdata?.udidcard?.length > 0 ? (
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <tbody>
                    {accessdata?.udidcard?.map((list,index)=>{
                        return(
<tr>
                        <td>{index + 1}</td>
                        <td>
    ...
    <button
      onClick={() => {
        secureLocalStorage.setItem("opudid", list?._id);
        Navigate('/opudiddetails');
      }}
      type="button"
...
export default Opservicelistudid;
